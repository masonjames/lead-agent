...
import { normalizeSarasotaPaoPropertyDetails } from "./normalize";
import {
...
export class SarasotaPaoAdapter implements ParcelSourceAdapter {
  key = SARASOTA_PAO_SOURCE_KEY;
  displayName = "Sarasota County Property Appraiser";
  config = SARASOTA_PAO_CONFIG;

  /**
   * Resolve: Find parcel detail URL from address
   *
   * For Sarasota PAO, we use the existing scraper's search functionality.
  ...
  async resolve(
    ...
    try {
      ...
      const result = await scrapeSarasotaPaoPropertyByAddressPlaywright(input.address, {
        timeoutMs: 60000,
        navTimeoutMs: 45000,
      });
      ...
      (this as SarasotaPaoAdapterWithCache)._cachedResult = result;
   ...
   *
   * For Sarasota PAO, the scraper already fetched the content during resolve.
  ...
  async fetch(
    ...
    const cached = (this as SarasotaPaoAdapterWithCache)._cachedResult;
   ...
   *
   * For Sarasota PAO, extraction is already done during resolve.
  ...
  async normalize(
    ...
    const normalized = normalizeSarasotaPaoPropertyDetails(raw, {
    ...
    // Clear cached result
    (this as SarasotaPaoAdapterWithCache)._cachedResult = undefined;
...
interface SarasotaPaoAdapterWithCache extends SarasotaPaoAdapter {
  _cachedResult?: {
...
export function createSarasotaPaoAdapter(): ParcelSourceAdapter {
  return new SarasotaPaoAdapter();
}
